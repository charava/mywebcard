import React from 'react';
import './App.css';

function BooksArchive() {
  return (
    <div className="App">
      <div className="main-container">
      <div>



      <p>books have very good nutritional value. i consume them as training data. if we're all just AI, might as well have high accuracy.</p>
      <p><small><i>i only recently started recording the exact date i finished reading the book. ive forgotten a lot of the books ive read in the past but here is a list of some.</i></small></p>

        <ul>
        <li>A View From the Stars - Cixin Liu - 8/18/24</li>
        <li>Tomorrow, and Tomorrow, and Tomorrow - Gabrielle Zevin - 8/10/24</li>
        <li>Brave New World - 7/31/24</li>
        <li>Factfulness - 7/24/24</li>
        <li>Sapiens - 7/19/24</li>
        <li>Doing Good Better - 7/12/24</li>
        <li>The Seven Husbands of Evelyn Hugo - 7/10/24</li>
        <li>The Night Watchman - 7/4/24</li>
        <li>My Year of Rest and Relaxation - 6/23/24</li>
        <li>The Light We Carry - 6/19/24</li>
        <li>Elon Musk - 6/9/24</li>
        <li>Bill Bryson - I’m a Stranger Here Myself - 2024</li>
        <li>The Great Gatsby - 2024</li>
        <li>Lord of the Flies - 7/23</li>
        <li>The Count of Monte Cristo - 7/23</li>
        <li>There There - 6/23</li>
</ul>
<ul>
        <li>Educated </li>
        <li>Girl in Pieces</li>
        <li>A Little Life</li>
        <li>Rationality</li>
        <li>The Ministry of the Future</li>
        <li>The Three Body Problem</li>
        <li>The Witch of Blackbird Pond</li>
        <li>Never Let Me Go</li>
        <li>Stutz - Therapist with Parkinson's Life</li>
        <li>The Reason I Jump - Autism Boy Life</li>
        <li>The Book Thief</li>
        <li>Ordinary Hazards</li>
        <li>Paper Towns</li>
        <li>Shout</li>
        <li>Weapons of Math Destruction</li>
        <li>Curious Incident of the Dog in the Night</li>
        <li>Olive’s Ocean</li>
        <li>Turtles All the Way Down</li>
        <li>Long Way Down</li>
        <li>Dead End in Norvelt</li>
        <li>Counting By Sevens</li>
        <li>Took</li>
        <li>The Way to Bea</li>
        <li>The Thing About Luck</li>
        <li>The Sky is Everywhere</li>
        <li>Between Shades of Grey</li>
        <li>Kira Kira</li>
        <li>The Mysterious Benedict Society</li>
        <li>Brown Girl Dreaming</li>
        <li>Watch Us Rise</li>
        <li>A Heart in a Body in a World</li>
        <li>The Hate U Give</li>
        <li>Wonder</li>
        <li>When You Reach Me</li>
        <li>Chains</li>
        <li>Everything Everything</li>
        <li>I’m Not Your Perfect Mexican Daughter</li>
        <li>Are You There God? It’s Me, Margaret</li>
        <li>The Girl Who Fell from the Sky</li>
        <li>Opposite of Always</li>
        <li>On the Come Up</li>
        <li>The Poet X</li>
        <li>Tell Me Three Things</li>
        <li>Holding Up the Universe</li>
        <li>Girl in Pieces</li>
        <li>Me and Earl and the Dying Girl</li>
        <li>A Very Large Expanse of Sea</li>
        <li>The 57 Bus</li>
        <li>We Are Okay</li>
        <li>Eleanor and Park</li>
        <li>We Were Liars</li>
        <li>I’ll Give You the Sun</li>
        <li>Far from the Tree</li>
        <li>All the Bright Places</li>
        <li>The Apology</li>
      </ul>
    </div>
      
      
      </div>
    </div>
  );
}

export default BooksArchive;
